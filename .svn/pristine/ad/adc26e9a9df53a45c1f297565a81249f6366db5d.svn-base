...
public class POClient2 extends Shell {
	...
	public void showPackingDetailInfo(){
		...
		if( items!=null ){
			...
			if( poCarton != null ) {
				...
				getInfoItemLbl().setText( String.valueOf( poCarton.getPd().getPurchaseOrderDetail().getUpc().getItem().getCode() ) );
	...
	public void loadPoDetailTable(){					//	fill purchase detail table
		...
		try{item = (Item)getItemCbx().getData(getItemCbx().getItem(getItemCbx().getSelectionIndex()));}catch(Exception e){}
		...
		for( PurchaseOrderDetail pod : po.getPurchaseOrderDetails() ){
			...
			if( item!=null && upc.getItem().getId()!=item.getId() ){
				tmpDet.remove(pod);
			}
		}
