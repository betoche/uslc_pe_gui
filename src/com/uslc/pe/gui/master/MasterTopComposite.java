...
import com.uslc.pe.gui.logic.NewPurchaseOrderLogic;
...
public class MasterTopComposite extends MasterSections {
		...
		if( print == null ) {
			...
			print.addSelectionListener( new SelectionAdapter() {
				...
				public void widgetSelected(SelectionEvent arg0) {
					if(getMaster().getMasterCenter().getPackingDetailViewer().isVisible()){
						//getMaster().getMasterCenter().getPackingDetailViewer().printTicket();
						getMaster().getMasterCenter().getPackingDetailViewer().exportToImage();
					}
				}
			} );
		}
	...
	public void showFileUploadDialog(){
		...
		if( path == null ){
			...
			Exception e = ((NewPurchaseOrderLogic)getMaster().getMasterCenter().getNewPurchaseOrder().getLiveDataAccessLifeCicle()).fillWithUploadedFile( path );
			if( e==null ){
				//getMaster().getMasterCenter().showComposite( getMaster().getMasterCenter().getNewPurchaseOrder() );
	...
	public Button getSavePo() {
		if( savePo == null ){
			...
			savePo.addSelectionListener(new SelectionAdapter() {
				...
				@Override
				public void widgetSelected(SelectionEvent arg0) {
					((NewPurchaseOrderLogic)getMaster().getMasterCenter().getNewPurchaseOrder().getLiveDataAccessLifeCicle()).savePo();
				}
			});
		}
		return savePo;
				...
				public void widgetSelected( SelectionEvent event ){
					...
					try{
						...
						for (TableItem item : items) {
							p = (PurchaseOrderStatus)item.getData();
						}
					
					
						if( p!=null && !p.getPo().getDeleted() ){
							com.uslc.pe.jpa.entity.PurchaseOrder po = p.getPo();
							...
							if( box.open()==SWT.YES ){
								...
								try {
									if(jpa.persist(po)){
										...
										getMaster().getMasterLeft().getLiveDataAccessLifeCicle().clean();
										getMaster().getMasterLeft().getLiveDataAccessLifeCicle().displayValues();
									}else{
