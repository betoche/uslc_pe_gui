...
public class UpcManagerComposite extends FormCenterMaster implements MasterCompositeInterface {
	...
	private void updateColorItemLabel() {
		...
		Style style = null;
		try{
			style = (Style)getItemsCmb().getData(getItemsCmb().getItem( getItemsCmb().getSelectionIndex() ));
		}catch( IllegalArgumentException e ){
		}
		try{ 
			getColorItemLbl().setText( color.getName()+"-"+style.getCode() ); 
	...
	private void performAction() throws Exception{
		...
		Style style = (Style)getItemsCmb().getData( getItemsCmb().getItem(getItemsCmb().getSelectionIndex()) );
		String colorItem = color.getName()+"-"+style.getCode();
	...
	private void setEditMode(){
		...
		if( upc!=null ){
			editing = true;
			selectedUpc = upc;
			getItemsCmb().setText( String.valueOf( upc.getStyle().getCode() ) );
	...
	public class UpcManagerCompositeLogic implements LiveDataAccessLifeCicle {
		...
		public void displayValues() {
			...
			for (Style style : styles) {
				getItemsCmb().add( String.valueOf( style.getCode() ) );
				getItemsCmb().setData(String.valueOf( style.getCode() ), style);
			}
