package com.uslc.pe.gui.report;
...
import com.uslc.pe.gui.report.PoComparativeByColorItemSize.ItemColorSizeCount.SizeCounts;
...
public class PoComparativeByColorItemSize {
	...
	public boolean containsItemColorSizeCount( Upc upc ){
		...
		for( ItemColorSizeCount icsc : getItemColorSizeCountList() ){
			...
			Style style = upc.getStyle();
			
			if( color.getId()==icsc.getColor().getId() && style.getId()==icsc.getStyle().getId() ){
				contains = true;
				break;
			}
		}
		...
		public class SizeCounts {
			...
			public SizeCounts( Size size ) {
				...
				for( PurchaseOrderDetail pod : getPo().getPurchaseOrderDetails() ) {
					Upc upc = pod.getUpc();
					if( getColor().getId()==upc.getColor().getId() && getStyle().getId()==upc.getStyle().getId() && size.getId()==upc.getSize().getId() ) {
