...
import com.uslc.pe.gui.client.Client;
import com.uslc.pe.gui.client.ClientPackingDetailTable;
import com.uslc.pe.gui.client.ClientPurchaseOrderDetailTable;
import com.uslc.pe.gui.client.ClientPurchaseOrderSumary;
import com.uslc.pe.gui.client.ClientScannedItemsTable;
import com.uslc.pe.gui.master.interfaces.LiveDataAccessLifeCicle;
import com.uslc.pe.gui.util.CommonMasterClient;
...
import com.uslc.pe.jpa.entity.PurchaseOrderByUser;
...
public class ClientLogic extends CommonMasterClient implements LiveDataAccessLifeCicle {
	...
	public void loadAvailablePurchaseOrders(){
		...
		for (PurchaseOrderByUser poByUser : purchaseOrdersByUser) {
			if( !poByUser.getDeleted() && !poByUser.getPurchaseOrder().getDeleted() ){
				com.uslc.pe.jpa.entity.PurchaseOrder po = poByUser.getPurchaseOrder();
	...
	public void fillFiltersFromPurchaseOrder() {
		if( getSelectedPurchaseOrder()!=null ) {
			...
			for( PurchaseOrderDetail pod : getSelectedPurchaseOrder().getPurchaseOrderDetails() ) {
				Upc upc = pod.getUpc();
				if( !styleList.contains( upc.getStyle() ) )styleList.add(upc.getStyle());
			...
			for (Style style : styleList) {
				getClient().getItemCbx().add( String.valueOf( style.getCode() ) );
				getClient().getItemCbx().setData( String.valueOf( style.getCode() ), style);
			}
	...
	public void showPurchaseOrderDetail(){
		...
		for( PurchaseOrderDetail pod : getSelectedPurchaseOrder().getPurchaseOrderDetails() ){
			if( !pod.getDeleted() ){
				Upc upc = pod.getUpc();
				boolean add = true;
				if( getSelectedStyle()!=null && upc.getStyle().getId()!=getSelectedStyle().getId() ){
					add = false;
				}
	...
	public void showScanningInfo(){
		if( getSelectedScannedItem()!=null ){
			...
			getClient().getInfoItemLbl().setText( String.valueOf( upc.getStyle().getCode() ) );
	...
	private Style getSelectedStyle(){
		...
		try {
			...
			if( c.getSelectionIndex()>-1 )
				style = (Style)c.getData( c.getItem( c.getSelectionIndex() ) );
		} catch( Exception e ) {
