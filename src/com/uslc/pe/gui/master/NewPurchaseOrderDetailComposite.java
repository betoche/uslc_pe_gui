...
public class NewPurchaseOrderDetailComposite extends FormCenterMaster {
	...
	private Style getSelectedStyle(){
		...
		try{
			style = (Style)cbx.getData(cbx.getItem(cbx.getSelectionIndex()));
		}catch( Exception e ){
			getLog().error(e);
		}
		return style;
	...
	public List<Upc> getUpcList(){
		if( upcList==null ){
			if( getMaster().getMasterCenter().getNewPurchaseOrder().getOrderNumberTxt().getText().endsWith( "11" ) ){
				upcList = UpcRepo.findByItemAndInseam(getSelectedStyle(), 36 );
			}else{
				upcList = UpcRepo.findByItem(getSelectedStyle());
			}
