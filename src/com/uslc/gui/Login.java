...
import com.uslc.pe.gui.master.Master;
import com.uslc.pe.jpa.entity.Log;
import com.uslc.pe.jpa.entity.User;
import com.uslc.pe.jpa.logic.Actions;
import com.uslc.pe.jpa.logic.Forms;
...
public class Login {
	...
	public void makeLogin(){
		...
		User user = null;
		try {
			user = getLiveDataAccess().getUslcData().getUser(username, password);
			...
			if( user==null ){
				...
				try{
					UslcJpa jpa = getLiveDataAccess().getUslcData().getJpa();
					if( user.getId()>0 ) {
				...
				if( user.getUserType()==UserType.ADMIN.getId() ){
					...
					gui = new Client( getShell(), getDisplay(), user );
